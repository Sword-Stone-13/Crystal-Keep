	...
	for(var/sound_range = 0 to range)
		...
		for(var/turf/stomp_turf in all_turfs)
			...
			for(var/mob/living/hit_mob in stomp_turf)
				...
				//var/turf/thrownat = get_ranged_target_turf_direct(owner, hit_mob, throw_range, rand(-10, 10))//ckeeptesting
				//hit_mob.throw_at(thrownat, 8, 2, null, TRUE, force = MOVE_FORCE_OVERPOWERING)//ckeeptesting
