...
/*	//Add xp based on the fatigue used //probably not needed, but left here for reference
	if(xp_gain)
		var/mob/living/carbon/human/mage = user
		add_sleep_experience(usr, associated_skill, round(get_fatigue_drain() * MAGIC_XP_MULTIPLIER *(mage.STACON / 10)))	//Formerly INT, now STACON because it's vital for mages. Maybe later to STASTR because of muscular philosophical brilliance 
*/
