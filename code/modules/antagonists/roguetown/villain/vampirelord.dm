...
/mob/living/carbon/human/proc/demand_submission()
	...
	if(istype(C))
		if(C.kingsubmit)
			to_chat(src, "I am already the Master of the Crystal Keep.")
			return
		...
		switch(choice)
			...
			if("Steal the Sun")
				...
				if(GLOB.tod == "night")
					...
					return
				if(alert(user, "Force Crystal Keep into Night? Cost:5000","","Yes","No") == "Yes")
...
/datum/objective/vampirelord/conquer
	name = "conquer"
	explanation_text = "Make the Ruler of Crystal Keep bow to my will."
...
/datum/objective/vampirelord/infiltrate/two/check_completion()
	var/datum/game_mode/chaosmode/C = SSticker.mode
	var/list/noblejobs = list("King", "Queen", "Heir", "Heiress", "Hand", "Steward")
	for(var/datum/mind/V in C.vampires)
...
/obj/effect/proc_holder/spell/targeted/transfix/cast(list/targets, mob/user = usr)
	...
	for(var/mob/living/carbon/human/L in targets)
		...
		var/willpower = round(L.STAFTH / 4)
