...
/obj/item/contraption/proc/misfire(obj/O, mob/living/user)
	user.mind.add_sleep_experience(/datum/skill/craft/engineering, (user.STASKL * 5))
...
/obj/item/contraption/wood_metalizer/attack_obj(obj/O, mob/living/user)
	...
	user.mind.add_sleep_experience(/datum/skill/craft/engineering, (user.STASKL / 2))
...
/obj/item/contraption/smelter/attack_obj(obj/O, mob/living/user)
	...
	if(!O.smeltresult)
		...
		S.start()
		return
	user.mind.add_sleep_experience(/datum/skill/craft/engineering, (user.STASKL / 3))
	charge_deduction(O, user, 1)
	flick(on_icon, src)
	...
	for(var/type in allowed_locks)
		if(istype(O, type))
			...
			if(mode == "Imprinter")
				...
				S.set_up(1, 1, front)
				S.start()
				user.mind.add_sleep_experience(/datum/skill/craft/engineering, (user.STASKL)) // Only imprinting gives EXP
