	...
	if(ismob(target))
		...
		if(M.client)
			if(M.can_see_cone(user))
				to_chat(M, span_danger("[user] bit their thumb at me!"))
		else
			M.taunted(user)
	return
...
/datum/intent/unarmed/help/rmb_ranged(atom/target, mob/user)
	if(ismob(target))
		var/mob/M = target
		var/list/targetl = list(target)
		user.visible_message(span_green("[user] points at [M]."), span_green("I point at [M]."), ignored_mobs = targetl)
		if(M.client)
			if(M.can_see_cone(user))
				to_chat(M, span_green("[user] points at me."))
	return
