...
/proc/pollGhostCandidates(Question, jobbanType, datum/game_mode/gametypeCheck, be_special_flag = 0, poll_time = 300, ignore_category = null, flashwindow = TRUE)
	...
	for(var/mob/living/carbon/spirit/challade in GLOB.player_list) //I don't know what this is, it had bigchungus on it, I assume it's the BSTONE coder's Ckey so I'm replacing it with my own to see what's up.
		candidates += challade
...
/proc/show_sensory_effect(atom/source, duration = 5, icon_state = "footstep", dir = null, ignore_self = FALSE)
	...
	addtimer(CALLBACK(GLOBAL_PROC, .proc/display_sensory_effect, source, T, duration, icon_state, dir, ignore_self), 0.5 SECONDS)
